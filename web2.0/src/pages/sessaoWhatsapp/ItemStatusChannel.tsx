...
import { CircularProgress, ListItem, ListItemAvatar, ListItemText, Avatar } from '@mui/material';
        ...
        <ListItem className="flex items-start">
            <ListItemAvatar>
                ...
                {item.status === 'CONNECTED' && (
                    <WifiLow name="WifiArrowUpDown" size={40} className="text-green-500" />
                )}
                {['PAIRING', 'TIMEOUT'].includes(item.status) && (
                    <WifiHighIcon name="WifiStrength1Alert" size={40} className="text-red-500" />
                )}
                {item.status === 'OPENING' && (
                    <CircularProgress className="text-green-700" size={48} thickness={2} />
                )}
            </ListItemAvatar>
                ...
                {item.status === 'CONNECTED' && ['whatsapp', 'baileys', 'waba'].includes(item.type) && (
                    <div className="flex items-center">
                        {item.profilePic !== 'disabled' ? (
                            ...
                            <MessageCircle name="Whatsapp" size={30} className="rounded-full mr-2" />
                        )}
